...
class WisataRouteOptimizer:
    def __init__(self):
        ...
        # Data destinasi (rating, biaya tiket, biaya parkir)
        self.destination_data = {
            'Malioboro': {'rating': 4.8, 'cost': 0, 'parking': 5000},
            'Taman Sari': {'rating': 4.6, 'cost': 25000, 'parking': 0},
            'Alun-Alun Kidul Jogja': {'rating': 4.7, 'cost': 0, 'parking': 5000},
            'Pasar Prawirotaman': {'rating': 4.5, 'cost': 0, 'parking': 5000},
            'Kaliurang - Jeep Tour': {'rating': 4.5, 'cost': 0, 'parking': 0},
            'Candi Prambanan': {'rating': 4.7, 'cost': 50000, 'parking': 0},
            'Goa Pindul': {'rating': 4.5, 'cost': 40000, 'parking': 0},
            'Pantai Parangtritis': {'rating': 4.6, 'cost': 0, 'parking': 5000},
            'Pantai Glagah': {'rating': 4.4, 'cost': 10000, 'parking': 5000},
            'Pedestrian Tugu dan Margo Utomo': {'rating': 4.8, 'cost': 0, 'parking': 5000}
        }
        ...
        self.weights = {
            ...
            'rating': 0.15,  # 15% - Rating destinasi
            'cost': 0.10,  # 10% - Biaya total
            'time_slot': 0.05  # 5% - Jam favorit
        }
    ...
    def calculate_total_cost_and_rating(self, route):
        total_cost = 0
        total_rating = 0
        for i in range(len(route) - 1):
            data = self.destination_data[route[i]]
            total_cost += data['cost'] + data['parking']
            total_rating += data['rating']
        return total_cost, total_rating

    def calculate_fitness(self, route):
        total_distance = self.calculate_total_distance(route)
        total_time = self.calculate_total_time(route)
        total_cost, total_rating = self.calculate_total_cost_and_rating(route)
        avg_rating = total_rating / len(self.locations)
        ...
        normalized_cost = 1 / (1 + total_cost / 100000)
        normalized_time_slot = 0.8
        ...
        combined_score = (
            ...
            self.weights['time_slot'] * normalized_time_slot
        )
